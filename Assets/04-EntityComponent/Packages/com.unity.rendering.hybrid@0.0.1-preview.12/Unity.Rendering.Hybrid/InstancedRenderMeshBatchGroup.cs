...
#pragma warning restore 649
    ...
    public unsafe class InstancedRenderMeshBatchGroup
    {
        ...
        public unsafe void AddBatch(FrozenRenderSceneTag tag, int rendererSharedComponentIndex, int batchInstanceCount, NativeArray<ArchetypeChunk> chunks, NativeArray<int> sortedChunkIndices, int startSortedIndex, int chunkCount, bool flippedWinding, EditorRenderData data)
        {
            ...
            float[] matParams = new float[batchInstanceCount];
            ...
            var mpbParams = m_ComponentSystem.GetArchetypeChunkComponentType<FloatMaterialPropertyBlock>(true);
            ...
            for (int i = 0; i < chunkCount; ++i)
            {
                ...
                fixed (float* matParamsPtr = &matParams[i * chunk.Count])
                {
                    var floatSizeOf = UnsafeUtility.SizeOf<float>();
                    var npbParamsNA = chunk.GetNativeArray(mpbParams);
                    float* srcNpb = (float*)npbParamsNA.GetUnsafeReadOnlyPtr();



                    UnsafeUtility.MemCpy(matParamsPtr, srcNpb, floatSizeOf * chunk.Count);
                }
            }

            mpb.SetFloatArray("FloatMPB", matParams);
            m_BatchRendererGroup.SetInstancingData(externalBatchIndex, batchInstanceCount, mpb);
